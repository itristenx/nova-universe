...
interface DashboardState {
  ...
  moveWidget: (id: string, position: { x: number; y: number; width: number; height: number }) => Promise<void>

  saveDashboardLayout: (layout: Omit<DashboardLayout, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>
  ...
  sendCollaborationEvent: (event: Omit<CollaborationEvent, 'id' | 'timestamp' | 'userId' | 'userName' | 'userAvatar'>) => Promise<void>
...
export const useDashboardStore = create<DashboardState>()(
  devtools(
    (set, get) => ({
      ...
      // Move widget
      moveWidget: async (id: string, position: { x: number; y: number; width: number; height: number }) => {
        const { widgets } = get()
      ...
      saveDashboardLayout: async (layout: Omit<DashboardLayout, 'id' | 'createdAt' | 'updatedAt'>) => {
        try {
      ...
      sendCollaborationEvent: async (event: Omit<CollaborationEvent, 'id' | 'timestamp' | 'userId' | 'userName' | 'userAvatar'>) => {
        try {
