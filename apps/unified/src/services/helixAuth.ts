...
class HelixAuthService {
  ...
  async authenticate(request: AuthenticationRequest): Promise<AuthenticationResponse> {
    ...
    const { data } = await api.post<AuthenticationApiResponse>(
  ...
  async verifyMfa(request: MfaVerificationRequest): Promise<AuthenticationResponse> {
    ...
    const { data } = await api.post<MfaVerificationApiResponse>(
   ...
   */
  async getSSOProviders(tenantId: string): Promise<
    Array<{
      provider: string;
      name: string;
      loginUrl: string;
  ...
  async handleSSOCallback(
    ...
    const response = await apiClient.post<AuthenticationResponse>(
      `${this.baseUrl}/sso/callback/${provider}`,
      {
        code,
        state,
      },
