...
export class SCIMService {
  ...
  static async triggerSync(): Promise<{ message: string; jobId: string }> {
    const response = await apiClient.post<{ message: string; jobId: string }>(`${this.baseUrl}/sync`)
    if (!response.data) {
   ...
   * Get sync job status
   */
  static async getSyncStatus(jobId: string): Promise<{ status: string; progress: number; message: string }> {
   ...
   * Import SCIM data
   */
  static async importData(file: File): Promise<{ message: string; imported: number; errors: string[] }> {
