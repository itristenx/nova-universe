...
class PulseService {
  ...
  async getDashboard(): Promise<PulseDashboardData> {
    const response = await apiClient.get<ApiResponse<{ dashboard: PulseDashboardData }>>(`${this.baseUrl}/dashboard`)
  ...
  async getQueueAgents(queueName: string): Promise<AgentAvailability[]> {
    const response = await apiClient.get<ApiResponse<{ agents: AgentAvailability[] }>>(`${this.baseUrl}/queues/${queueName}/agents`)
   ...
   */
  async updateAgentAvailability(queueName: string, data: {
    isAvailable: boolean
    status?: string
    maxCapacity?: number
  }): Promise<void> {
    await apiClient.post<ApiResponse<void>>(`${this.baseUrl}/queues/${queueName}/agents/availability`, data)
  }
