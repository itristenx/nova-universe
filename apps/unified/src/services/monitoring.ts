...
export const monitoringService = {
  ...
  async getMonitor(id: string): Promise<Monitor> {
    const response = await apiClient.get<Monitor>(`/api/v1/monitoring/monitors/${id}`);
    return response.data!;
  },

  async createMonitor(data: Omit<Monitor, 'id' | 'createdAt' | 'updatedAt'>): Promise<Monitor> {
    const response = await apiClient.post<Monitor>('/api/v1/monitoring/monitors', data);
    return response.data!;
  },

  async updateMonitor(id: string, data: Partial<Monitor>): Promise<Monitor> {
    const response = await apiClient.patch<Monitor>(`/api/v1/monitoring/monitors/${id}`, data);
    return response.data!;
  },

  async deleteMonitor(id: string): Promise<void> {
    await apiClient.delete(`/api/v1/monitoring/monitors/${id}`);
  },

  async pauseMonitor(id: string): Promise<void> {
    await apiClient.post(`/api/v1/monitoring/monitors/${id}/pause`);
  },

  async resumeMonitor(id: string): Promise<void> {
    await apiClient.post(`/api/v1/monitoring/monitors/${id}/resume`);
  },

  async testMonitor(id: string): Promise<MonitorResult> {
    const response = await apiClient.post<MonitorResult>(`/api/v1/monitoring/monitors/${id}/test`);
    return response.data!;
  },
  ...
  async getMonitorResults(
    ...
    const response = await apiClient.get<MonitorResult[]>(
      `/api/v1/monitoring/monitors/${monitorId}/results`,
      {
        params: { timeRange },
      },
    );
  ...
  async getMonitorUptime(
    ...
    const response = await apiClient.get<{
      ...
      successfulChecks: number;
      averageResponseTime: number;
    }>(`/api/v1/monitoring/monitors/${monitorId}/uptime`, {
      params: { timeRange },
    });
    return response.data!;
  },
  ...
  async getAlerts(monitorId?: string): Promise<PaginatedResponse<Alert>> {
    ...
    return await apiClient.getPaginated<Alert>('/api/v1/monitoring/alerts', params);
  },

  async acknowledgeAlert(id: string): Promise<void> {
    await apiClient.post(`/api/v1/monitoring/alerts/${id}/acknowledge`);
  },

  async resolveAlert(id: string): Promise<void> {
    await apiClient.post(`/api/v1/monitoring/alerts/${id}/resolve`);
  },
  ...
  async getStats(): Promise<MonitorStats> {
    const response = await apiClient.get<MonitorStats>('/api/v1/monitoring/stats');
    return response.data!;
  },

  // Status pages
  async getStatusPages(): Promise<StatusPage[]> {
    const response = await apiClient.get<StatusPage[]>('/api/v1/monitoring/status-pages');
    return response.data!;
  },

  async getStatusPage(slug: string): Promise<StatusPage> {
    const response = await apiClient.get<StatusPage>(`/api/v1/monitoring/status-pages/${slug}`);
    return response.data!;
  },
  ...
  async createStatusPage(
    ...
    const response = await apiClient.post<StatusPage>('/api/v1/monitoring/status-pages', data);
    return response.data!;
  },

  async updateStatusPage(id: string, data: Partial<StatusPage>): Promise<StatusPage> {
    const response = await apiClient.patch<StatusPage>(`/api/v1/monitoring/status-pages/${id}`, data);
    return response.data!;
  },

  async deleteStatusPage(id: string): Promise<void> {
    await apiClient.delete(`/api/v1/monitoring/status-pages/${id}`);
  },

  // Incidents
  async createIncident(data: Omit<Incident, 'id' | 'createdAt' | 'updatedAt'>): Promise<Incident> {
    const response = await apiClient.post<Incident>('/api/v1/monitoring/incidents', data);
    return response.data!;
  },

  async updateIncident(id: string, data: Partial<Incident>): Promise<Incident> {
    const response = await apiClient.patch<Incident>(`/api/v1/monitoring/incidents/${id}`, data);
    return response.data!;
  },

  async resolveIncident(id: string): Promise<void> {
    await apiClient.post(`/api/v1/monitoring/incidents/${id}/resolve`);
  },
  ...
  async createMaintenance(
    ...
    const response = await apiClient.post<Maintenance>('/api/v1/monitoring/maintenance', data);
    return response.data!;
  },

  async updateMaintenance(id: string, data: Partial<Maintenance>): Promise<Maintenance> {
    const response = await apiClient.patch<Maintenance>(`/api/v1/monitoring/maintenance/${id}`, data);
    return response.data!;
  },

  async startMaintenance(id: string): Promise<void> {
    await apiClient.post(`/api/v1/monitoring/maintenance/${id}/start`);
  },

  async completeMaintenance(id: string): Promise<void> {
    await apiClient.post(`/api/v1/monitoring/maintenance/${id}/complete`);
  },
