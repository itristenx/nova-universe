...
export const monitoringService = {
  ...
  async getMonitors(filters?: MonitorFilters): Promise<PaginatedResponse<Monitor>> {
    ...
    return await apiClient.getPaginated<Monitor>('/v1/monitoring/monitors', Object.fromEntries(params))
  },
  ...
  async getMonitorResults(monitorId: string, timeRange: '1h' | '24h' | '7d' | '30d' = '24h'): Promise<MonitorResult[]> {
    const response = await apiClient.get<MonitorResult[]>(`/v1/monitoring/monitors/${monitorId}/results`, {
      params: { timeRange }
    })
    return response.data!
  },

  async getMonitorUptime(monitorId: string, timeRange: '24h' | '7d' | '30d' | '90d' = '30d'): Promise<{
    uptime: number
    totalChecks: number
  ...
  async createStatusPage(data: Omit<StatusPage, 'id' | 'createdAt' | 'updatedAt'>): Promise<StatusPage> {
  ...
  // Maintenance windows
  async createMaintenance(data: Omit<Maintenance, 'id' | 'createdAt' | 'updatedAt'>): Promise<Maintenance> {
  ...
  async subscribeToMonitorEvents(callback: (event: {
