...
class NovaTVService {
  ...
  async reorderContent(dashboardId: string, contentIds: string[]): Promise<Content[]> {
    const response = await api.post(`/nova-tv/dashboards/${dashboardId}/content/reorder`, { contentIds })
    return response.data
  }
  ...
  async getDashboardAnalytics(
    ...
    const response = await api.get(`/nova-tv/analytics/dashboard/${dashboardId}`, { params: filters })
    return response.data
  }
