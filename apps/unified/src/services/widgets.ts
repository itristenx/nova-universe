...
class WidgetService {
  ...
  async updateDashboardLayout(id: string, updates: Partial<DashboardLayout>): Promise<DashboardLayout> {
  ...
  async createDashboardFromTemplate(templateId: string, name: string, userId?: string): Promise<DashboardLayout> {
  ...
  async sendCollaborationEvent(event: Omit<CollaborationEvent, 'id' | 'timestamp' | 'userId' | 'userName' | 'userAvatar'>): Promise<void> {
