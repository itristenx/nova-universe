...
export interface UptimeKumaMaintenance {
  ...
  title: string
  description: string
  strategy: 'single' | 'recurring-interval' | 'recurring-weekday' | 'recurring-day-of-month' | 'manual' | 'cron'
  active: boolean
...
export const uptimeKumaService = {
  ...
  async createMonitor(monitor: Omit<NovaMonitor, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'uptime24h' | 'uptime7d' | 'uptime30d' | 'averageResponseTime'>): Promise<NovaMonitor> {
    const kumaMonitor = transformNovaMonitorToKuma(monitor)
    const response = await apiClient.post<UptimeKumaMonitor>('/v1/uptime-kuma/monitors', kumaMonitor)
    return transformUptimeKumaMonitor(response.data!)
  },
  ...
  async updateMonitor(id: string, updates: Partial<NovaMonitor>): Promise<NovaMonitor> {
    ...
    const response = await apiClient.patch<UptimeKumaMonitor>(`/v1/uptime-kuma/monitors/${id}`, kumaUpdates)
    return transformUptimeKumaMonitor(response.data!)
  },
  ...
  async testMonitor(id: string): Promise<NovaHeartbeat> {
    const response = await apiClient.post<UptimeKumaHeartbeat>(`/v1/uptime-kuma/monitors/${id}/test`)
    return transformUptimeKumaHeartbeat(response.data!)
  },

  // Monitor Data & Analytics
  async getMonitorHeartbeats(monitorId: string, period: '1h' | '24h' | '7d' | '30d' = '24h'): Promise<NovaHeartbeat[]> {
    const response = await apiClient.get<UptimeKumaHeartbeat[]>(`/v1/uptime-kuma/monitors/${monitorId}/heartbeats`, {
      params: { period }
    })
    return response.data!.map(transformUptimeKumaHeartbeat)
  },

  async getMonitorUptime(monitorId: string, period: '24h' | '7d' | '30d' | '90d' = '30d'): Promise<{
    uptime: number
    totalChecks: number
    successfulChecks: number
    averageResponseTime: number
  }> {
    const response = await apiClient.get<UptimeKumaUptimeData>(`/v1/uptime-kuma/monitors/${monitorId}/uptime`, {
      params: { period }
    })
  ...
  // Status Pages
  async getStatusPages(): Promise<Array<{
    id: string
    slug: string
    title: string
    description?: string
  ...
  // Real-time Events
  async subscribeToEvents(callback: (event: {
