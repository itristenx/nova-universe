...
export function useTouchGestures(
    ...
    setTouchState(current => {
      ...
      if (remainingTouches.length === 0 && current.touches.length === 1) {
        ...
        if (current.isPanning) {
          ...
          if (distance < optionsRef.current.swipeThreshold && duration < optionsRef.current.tapTimeout) {
            // Tap gesture
            ...
            if (velocity >= optionsRef.current.velocityThreshold) {
              const gesture = createGesture('swipe', startTouch, endTouch, duration, { direction })
  ...
  useEffect(() => {
    ...
    element.addEventListener('touchstart', handleTouchStart, { passive: !optionsRef.current.preventDefault })
    element.addEventListener('touchmove', handleTouchMove, { passive: !optionsRef.current.preventDefault })
    element.addEventListener('touchend', handleTouchEnd, { passive: !optionsRef.current.preventDefault })
        ...
        >
          {leftActions.map((action, index) => (
            <button
              ...
              className={`h-full px-4 flex items-center space-x-2 text-white font-medium ${
