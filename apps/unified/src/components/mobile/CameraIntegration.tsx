        ...
        (blob) => {
          if (blob) {
            ...
            if (blob.size > maxFileSize) {
              const error = new Error(t('app.camera.fileSizeExceeded', { size: maxFileSize / (1024 * 1024) }))
              setError(error.message)
              onError?.(error)
              return
            }
  ...
  if (!capabilities) {
    return (
      <div className={`flex items-center space-x-2 ${className}`}>
        <button
          type="button"
          className="flex items-center space-x-2 px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 rounded-lg cursor-not-allowed"
          disabled
        >
          ...
          <div className="relative w-full h-full max-w-4xl max-h-screen p-4 flex flex-col">
            ...
            <div className="flex items-center justify-center mt-4 space-x-4">
              <button
                ...
                className="w-16 h-16 bg-white hover:bg-gray-100 text-gray-900 rounded-full flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed border-4 border-gray-300"
              >
                {isCapturing ? (
