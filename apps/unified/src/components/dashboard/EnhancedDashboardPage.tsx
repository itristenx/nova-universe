...
export function EnhancedDashboardPage({ role = 'admin', className }: EnhancedDashboardPageProps) {
  ...
  const handleWidgetRefresh = (id: string) => {
    setWidgets(
      widgets.map((w) => (w.id === id ? { ...w, lastUpdated: new Date(), isLoading: false } : w)),
    );
  };
  ...
  const renderWidget = (widget: any) => {
    ...
    return (
      <div
        ...
        className={cn(
          'group relative cursor-pointer rounded-lg border border-gray-200 bg-white shadow-sm transition-all duration-200 dark:border-gray-700 dark:bg-gray-800',
          {
            'ring-opacity-50 ring-2 ring-blue-500': isSelected,
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
          <div>
            ...
            {widget.description && (
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{widget.description}</p>
            )}
          </div>
        ...
        {/* Widget Content */}
        <div className="h-full flex-1 p-4">{renderWidgetContent(widget)}</div>

        {/* Last Updated */}
        {widget.lastUpdated && (
          <div className="absolute right-2 bottom-2 text-xs text-gray-400 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        return (
          <div className="flex h-full flex-col justify-center text-center">
            <div className="mb-2 text-3xl font-bold text-gray-900 dark:text-white">
              ...
              {metric.unit && <span className="ml-1 text-lg text-gray-500">{metric.unit}</span>}
            </div>
            {metric.trendValue && (
            ...
            <div className="flex h-full flex-col justify-center">
              ...
              {alert.actionUrl && alert.actionLabel && (
                <button
                  ...
                  className="mt-3 inline-flex items-center rounded-md border border-current px-3 py-1 text-xs font-medium"
                >
                  {alert.actionLabel}
                </button>
              )}
  ...
  return (
    <div className={cn('h-full', className)}>
      ...
      {/* Widget Grid */}
      <div className="grid auto-rows-[180px] grid-cols-12 gap-4">{widgets.map(renderWidget)}</div>

      {/* Edit Mode Instructions */}
      {isEditMode && (
        <div className="mt-6 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
          <p className="text-sm text-blue-800 dark:text-blue-300">
            <strong>Edit Mode Active:</strong> Click widgets to select them, use the buttons to
            refresh or delete widgets. Click "View Mode" when you're done editing.
          </p>
        </div>
      )}
    </div>
