  ...
  if (widget) {
    ...
    return (
      <div
        ...
        onClick={handleClick}
        className={`
          relative bg-white dark:bg-gray-800 border rounded-lg p-4 shadow-sm
            ...
            <div className="text-center">
              ...
              {widget.config.metric.trend && (
                <div className={`text-sm mt-1 ${
                  widget.config.metric.trend === 'up' ? 'text-green-600' :
                  widget.config.metric.trend === 'down' ? 'text-red-600' :
                  'text-gray-600'
                }`}>
                ...
                <thead>
                  <tr className="border-b">
                    {widget.config.table.columns?.map((column, i) => (
                      <th key={i} className="text-left py-1 px-2">{column.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {widget.config.table.data?.slice(0, widget.config.table.maxRows || 10).map((row, i) => (
                    <tr key={i} className="border-b">
                      {widget.config.table?.columns?.map((column, j) => (
                        <td key={j} className="py-1 px-2">{row[column.key]}</td>
                      )) || []}
                    </tr>
                  ))}
                </tbody>
