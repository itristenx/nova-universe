  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    for (const file of filesToUpload) {
      if (file.size > maxFileSize) {
        setErrors(prev => [...prev, `File "${file.name}" is too large (max ${formatFileSize(maxFileSize)})`])
        continue
      }
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <div
        ...
        className={cn(
          ...
          (disabled || uploading || uploadedFiles.length >= maxFiles) && 'cursor-not-allowed opacity-50',
        ...
        <div className="flex flex-col items-center">
          ...
          {isDragActive ? (
            isDragReject ? (
              ...
              <p className="text-sm text-nova-600 dark:text-nova-400">
                Drop the files here...
              </p>
            )
          ) : (
      ...
      {errors.length > 0 && (
        <div className="space-y-2">
          {errors.map((error, index) => (
            <div key={index} className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-950 rounded-lg">
