...
export function MobileLayout({ children, sidebar, header, className }: MobileLayoutProps) {
  ...
  return (
    <div className={cn('min-h-screen bg-gray-50 dark:bg-gray-900', className)}>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-white dark:bg-gray-800 transition-transform duration-300 ease-in-out',
          isMobile
            ? isSidebarOpen
              ? 'translate-x-0'
              : '-translate-x-full'
      ...
      {isMobile && !isSidebarOpen && (
        <div className="fixed bottom-4 left-4 z-30">
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-600 text-white shadow-lg hover:bg-blue-700 active:scale-95 transition-all"
