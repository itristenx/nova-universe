...
export default function EnhancedUserDashboard() {
  ...
  const quickActions: QuickAction[] = [
    ...
    },
    {
      ...
      color: 'bg-gradient-to-r from-purple-50 to-pink-50 text-purple-700 hover:from-purple-100 hover:to-pink-100 border-purple-200',
      popular: true,
    },
    {
      id: 'cosmo-chat',
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'ticket_updated': return 'text-green-600'
      case 'service_requested': return 'text-purple-600'
      case 'knowledge_viewed': return 'text-orange-600'
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.completedTickets}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3">
            {notifications.slice(0, 3).map((notification) => (
              <div
                key={notification.id}
                className={`border-l-4 pl-4 py-2 rounded-r ${getNotificationColor(notification.type)}`}
              >
          ...
          <div className="space-y-4">
            {recentActivity.map((activity) => (
              <div key={activity.id} className="flex items-start space-x-3">
                <div className={`flex-shrink-0 p-2 rounded-lg ${getActivityIcon(activity.type)} bg-gray-100 dark:bg-gray-700`}>
                  {activity.icon}
                </div>
                <div className="flex-1 min-w-0">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-4">
            {recommendations.map((recommendation) => (
              ...
              >
                <div className="flex items-start space-x-3">
                  <div className="flex-shrink-0 p-2 bg-gradient-to-r from-purple-100 to-pink-100 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg">
                    {recommendation.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-sm text-gray-900 dark:text-white">
