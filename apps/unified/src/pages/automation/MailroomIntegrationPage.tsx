...
export default function MailroomIntegrationPage() {
  ...
  const [activeTab, setActiveTab] = useState<'inbox' | 'processed' | 'rules' | 'analytics'>('inbox')
      ...
      setEmails(prev => prev.map(email => 
        email.id === emailId 
          ? { 
              ...
              automationResult: {
                ticketCreated: `REQ-2024-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-xl px-6 py-8 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900 dark:text-white">{unprocessedCount}</div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900 dark:text-white">{processingCount}</div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900 dark:text-white">{processedCount}</div>
      ...
      {(activeTab === 'inbox' || activeTab === 'processed') && (
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-500">
                {emails.filter(e => activeTab === 'inbox' ? ['unprocessed', 'processing', 'manual_review'].includes(e.status) : e.status === 'processed').length} {t('mailroom:emails')}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {emails
              .filter(email => activeTab === 'inbox' ? ['unprocessed', 'processing', 'manual_review'].includes(email.status) : email.status === 'processed')
              .map((email) => (
              <div
                key={email.id}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(email.status)}`}>
                          {t(`mailroom:status.${email.status}`)}
                        </span>
                        {email.extractedData?.priority && (
                          <span className={`px-2 py-1 rounded-full text-xs ${getPriorityColor(email.extractedData.priority)}`}>
                      ...
                      {email.flags.length > 0 && (
                        <div className="mt-2 flex items-center space-x-2">
                          {email.flags.map((flag, index) => (
                            <span
                              key={index}
                              className="inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"
                            >
                  ...
                  {email.status === 'unprocessed' && (
                    <button
                      ...
                      className="px-3 py-1 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors"
                    >
                      {isProcessing ? (
                        <div className="flex items-center space-x-1">
                          <LoadingSpinner size="sm" />
          ...
          <div className="space-y-4">
            {processingRules.map((rule) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs text-gray-500 pt-3 border-t border-gray-200 dark:border-gray-600">
                  <div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <strong>{t('mailroom:from')}:</strong> {selectedEmail.from.name} ({selectedEmail.from.email})
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className={`ml-2 px-2 py-1 rounded-full text-xs ${getStatusColor(selectedEmail.status)}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      {selectedEmail.extractedData.priority && (
                        <div>
                          ...
                          <span className={`ml-2 px-2 py-1 rounded-full text-xs ${getPriorityColor(selectedEmail.extractedData.priority)}`}>
                      ...
                      {selectedEmail.extractedData.department && (
                        <div>
                          <strong>{t('mailroom:department')}:</strong> {selectedEmail.extractedData.department}
                        </div>
                      )}
                      {selectedEmail.extractedData.keywords && (
                        <div className="col-span-2">
                          <strong>{t('mailroom:keywords')}:</strong> {selectedEmail.extractedData.keywords.join(', ')}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-600 pt-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      {selectedEmail.automationResult.ticketCreated && (
                        <div>
                          <strong>{t('mailroom:ticketCreated')}:</strong> {selectedEmail.automationResult.ticketCreated}
                        </div>
                      )}
                      {selectedEmail.automationResult.assignedTo && (
                        <div>
                          <strong>{t('mailroom:assignedTo')}:</strong> {selectedEmail.automationResult.assignedTo}
                        </div>
                      )}
                      {selectedEmail.automationResult.actions && (
                        <div className="col-span-2">
                          <strong>{t('mailroom:actionsPerformed')}:</strong> {selectedEmail.automationResult.actions.join(', ')}
                        </div>
                      )}
                    </div>
                  </div>
