...
export const EnhancedQueueSwitcher: React.FC<Props> = ({ 
  ...
  const loadQueueData = async (silent = false) => {
    try {
      ...
      const user = (agentsData as AgentAvailability[]).find((agent: AgentAvailability) => agent.userId === userId)
  ...
  const determineQueueHealth = (metrics: QueueMetrics): 'excellent' | 'good' | 'warning' | 'critical' => {
  ...
  return (
    ...
    <div className="agents-loaded-info">
      ...
      <div className="bg-white border rounded-lg p-4">
        ...
        {showQueueSuggestions && skillRecommendations.length > 0 && (
          <div className="mt-3 p-3 bg-blue-50 rounded-md">
            <h4 className="text-sm font-medium text-blue-900 mb-2">Recommended Queues</h4>
            <div className="space-y-1">
              {skillRecommendations.slice(0, 3).map((rec) => (
                <button
                  key={rec.queueId}
                  onClick={() => handleQueueChange(rec.queueId)}
                  className="block w-full text-left text-xs p-2 bg-white rounded border hover:bg-blue-50"
                >
        ...
        <div className="bg-white border rounded-lg p-4">
          ...
          <div className="mb-4">
            ...
            <div className="capacity-bar-container">
              ...
              <div 
                className="sla-threshold-indicator"
                title="SLA Threshold"
              />
            </div>
          </div>
            ...
            <div className="border-t pt-3">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={`text-xs px-2 py-1 rounded-full ${
                  metrics.slaStatus.breachRisk >= 70 ? 'bg-red-100 text-red-800' :
                  metrics.slaStatus.breachRisk >= 40 ? 'bg-yellow-100 text-yellow-800' :
      ...
      {currentUser && (
        <div className="bg-white border rounded-lg p-4">
          ...
          {!currentUser.isAvailable && (
            <div className="mt-3">
              <label className="block text-xs text-gray-600 mb-1">Break Reason</label>
              <select
      ...
      {showComparison && (
        <div className="bg-white border rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-900 mb-3">Queue Comparison</h3>
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {queues.map((queue) => (
              <div key={queue} className="flex justify-between items-center p-2 border rounded">
