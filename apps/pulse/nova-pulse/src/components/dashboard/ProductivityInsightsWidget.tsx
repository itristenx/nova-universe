...
const MetricRow = ({ metric }: { metric: ProductivityMetric }) => {
  ...
  const progressColor = isOnTarget
    ? 'success'
  ...
  return (
    <div className="space-y-2">
      ...
      <Progress value={progressValue} color={progressColor} size="sm" className="w-full" />
    </div>
  );
};
  ...
  return (
    <div className="flex items-center justify-between py-2">
      <div className="flex flex-1 items-center space-x-3">
        <div
          className={`h-3 w-3 rounded-full ${colorClasses[distribution.color as keyof typeof colorClasses] || 'bg-gray-500'}`}
        />
        <span className="text-sm text-gray-700 dark:text-gray-300">{distribution.category}</span>
      </div>
      <div className="flex items-center space-x-2">
        ...
        <CardBody className="pt-0">
          <div className="space-y-3">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm text-gray-700 dark:text-gray-300">
                  <span className="font-medium">Above target</span> on customer satisfaction this
                  week
                </p>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="text-sm text-gray-700 dark:text-gray-300">
                  <span className="font-medium">Room for improvement</span> in first contact
                  resolution
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">
