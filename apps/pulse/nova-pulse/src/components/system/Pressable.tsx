...
const Pressable: React.FC<PressableProps> = ({ as = 'button', className = '', children, ...props }) => {
  ...
  const createRipple = (event: React.MouseEvent | React.TouchEvent) => {
    ...
    const x = ('clientX' in event ? event.clientX : (event as any).touches?.[0]?.clientX) - rect.left - size / 2
    const y = ('clientY' in event ? event.clientY : (event as any).touches?.[0]?.clientY) - rect.top - size / 2
