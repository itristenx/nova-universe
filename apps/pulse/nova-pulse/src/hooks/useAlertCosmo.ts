...
export const _useAlertCosmo = ({
    ...
    mutationFn: async (analysisContext: CosmoAlertContext & { message?: string }) => {
      const response = await fetch('/api/v2/synth/alerts/analyze', {
        ...
        body: JSON.stringify({
          ...
          message: analysisContext.message || 'Please analyze this situation and recommend alert actions',
          module: 'pulse',
  ...
  const analyzeWithCosmo = useCallback(async (
    analysisContext: CosmoAlertContext,
    message?: string
  ...
  const analyzeTicket = useCallback(async (ticketData: {
    id: string;
    title: string;
    description: string;
    priority: string;
  ...
  const analyzeEscalation = useCallback(async (alertData: Alert): Promise<CosmoAlertResponse> => {
    ...
    return analyzeWithCosmo({
      ...
      timeSinceAcknowledged ? `, acknowledged ${Math.floor(timeSinceAcknowledged / 60000)} minutes ago` : ', not yet acknowledged'
  ...
  const extractKeywords = (text: string): string[] => {
    const alertKeywords = [
      ...
      'security', 'breach', 'malware', 'phishing', 'unauthorized',
      'performance', 'slow', 'timeout', 'latency', 'degraded',
      'network', 'connectivity', 'disconnected', 'unreachable',
      'server', 'database', 'application', 'service', 'infrastructure'
    ];

    const words = text.toLowerCase().split(/\s+/);
    return alertKeywords.filter(keyword => 
