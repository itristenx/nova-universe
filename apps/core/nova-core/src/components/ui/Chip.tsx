...
export const Chip: React.FC<ChipProps> = ({
  ...
  const variantClass =
    variant === 'bordered' ? `border ${p.border} ${p.text}` :
    variant === 'solid' ? `${p.bg} ${p.text}` :
    variant === 'light' ? `bg-transparent ${p.text}` :
    `${p.bg} ${p.text}`;

  return (
    <div {...props} className={`${base} ${sizeClass} ${variantClass} ${props.className || ''}`.trim()}>
      {children ?? label}
    </div>
  );
};
