...
export const AdminPinManagement: React.FC<AdminPinManagementProps> = ({ kiosks, onUpdate }) => {
  ...
  const validatePin = (
    pin: string,
    context?: { type: 'global' | 'kiosk'; kioskId?: string },
  ): PinValidation => {
    if (pin.length === 0) {
    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <KeyIcon className="text-primary-600 h-6 w-6" />
            <div>
              <h3 className="text-lg font-medium text-gray-900">Admin PIN Management</h3>
              <p className="text-sm text-gray-600">
                Configure PINs for offline admin access on kiosks
              </p>
            </div>
          </div>
          <div className="flex space-x-3">
            ...
            <Button onClick={savePinConfiguration} disabled={loading}>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg border border-gray-200 p-4">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-gray-600">
                The global PIN works on all kiosks and provides full admin permissions including
                user management.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-4">
              {kiosks.length === 0 ? (
                <div className="rounded-md bg-gray-50 p-6 text-center">
                  <ComputerDesktopIcon className="mx-auto mb-4 h-12 w-12 text-gray-400" />
                  <h4 className="mb-2 text-sm font-medium text-gray-900">No Kiosks Available</h4>
                  <p className="text-sm text-gray-500">
                    No kiosks are currently registered in the system. Kiosk-specific PINs will be
                    available once kiosks are added.
                  </p>
                </div>
              ) : (
                kiosks.map((kiosk) => (
                  <div key={kiosk.id} className="rounded-md bg-gray-50 p-3">
                    ...
                    {pinConfig.kioskPins[kiosk.id] && (
                      <div className="mt-2 text-sm">
                        {validatePin(pinConfig.kioskPins[kiosk.id], {
                          type: 'kiosk',
                          kioskId: kiosk.id,
                        }).isValid ? (
                          <div className="flex items-center text-green-600">
              ...
              <p className="text-sm text-gray-600">
                Kiosk-specific PINs provide limited admin permissions (status, schedule, branding)
                for that specific kiosk only.
              </p>
            </div>
          </div>
        </div>
      ...
      >
        <div className="space-y-4">
          ...
          {testResult && (
            <div
              className={`rounded-md p-3 ${testResult.valid ? 'border border-green-200 bg-green-50' : 'border border-red-200 bg-red-50'}`}
            >
              <div className="mb-2 flex items-center space-x-2">
                ...
                <span
                  className={`font-medium ${testResult.valid ? 'text-green-800' : 'text-red-800'}`}
                >
              ...
              {testResult.valid && (
                <div className="space-y-1 text-sm">
                  <div>
                    <strong>PIN Type:</strong> {testResult.pinType || 'Unknown'}
                  </div>
                  <div>
                    <strong>Permissions:</strong> {testResult.permissions.join(', ')}
                  </div>
                  <div>
                    <strong>Expires:</strong> {new Date(testResult.expiresAt).toLocaleString()}
                  </div>
                </div>
              )}
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <Button onClick={validatePinInput} disabled={testPinValue.length !== 6 || loading}>
