...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedFormats[file.mimetype]) {
    ...
    cb(
      new Error(
        `Unsupported file format: ${file.mimetype}. Supported formats: ${Object.keys(allowedFormats).join(', ')}`,
      ),
    );
  }
};
...
function validateFileForDigitalSignage(file, fileInfo) {
  ...
  if (file.size > maxSizes[fileInfo.type]) {
    validations.push(
      `File size ${(file.size / 1024 / 1024).toFixed(2)}MB exceeds limit for ${fileInfo.type} files`,
    );
  }
      ...
      try {
        ...
        try {
          ...
          uploadResults.push({
            ...
            warnings: [
              ...(validation.warnings || []),
              'Database save failed - file uploaded but not tracked',
            ],
          });
        }
      } catch (fileError) {
