    ...
    try {
      ...
      if (downMonitors.length > 0) {
        recommendations.push(
          `Investigate ${downMonitors.length} down monitor${downMonitors.length > 1 ? 's' : ''}: ${downMonitors
            .map((m: any) => m.name)
            .slice(0, 3)
            .join(', ')}`,
        );
      ...
      if (criticalIncidents.length > 0) {
        recommendations.push(
          `Address ${criticalIncidents.length} critical incident${criticalIncidents.length > 1 ? 's' : ''} immediately`,
        );
      }

      if (unresolvedIncidents.length > 0) {
        recommendations.push(
          `Follow up on ${unresolvedIncidents.length} unresolved incident${unresolvedIncidents.length > 1 ? 's' : ''}`,
        );
      }
      ...
      if (systemEfficiency > 98) {
        recommendations.push(
          'System performing optimally - consider expanding monitoring coverage',
        );
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const criticalIncidents = incidents.filter((i: any) => i.severity === 'critical');
      const highPriorityIncidents = incidents.filter((i: any) =>
      ...
      if (oldIncidents.length > 0) {
        recommendedActions.push(
          `Review ${oldIncidents.length} long-running incident${oldIncidents.length > 1 ? 's' : ''}`,
        );
      }
