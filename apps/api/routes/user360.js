...
async function getIntegrationLayer() {
  if (!nilPromise) {
    nilPromise = import('../../lib/integration/nova-integration-layer.js')
      ...
      .catch((err) => {
        logger.warn('Integration layer unavailable; continuing in degraded mode', {
          error: err?.message,
        });
        return null;
      });
      ...
      switch (category) {
        ...
        case 'alerts':
          ...
          if (userRole === 'admin' || userRole === 'technician') {
            ...
            filtered[category] = profile[category].filter(
              (alert) => alert.severity !== 'critical' && !alert.securityRelated,
            );
          }
          break;
