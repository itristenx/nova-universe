...
router.get('/proxies', authenticateJWT, createRateLimit(15 * 60 * 1000, 50), async (req, res) => {
  try {
    const rows = await db.any('SELECT vp.*, u.name AS proxy_name FROM vip_proxies vp JOIN users u ON vp.proxy_id = u.id'); // TODO-LINT: move to async function
...
router.post('/proxies',
  ...
  async (req, res) => {
    try {
      ...
      await db.none('INSERT INTO vip_proxies (vip_id, proxy_id, created_at, expires_at) VALUES ($1,$2, CURRENT_TIMESTAMP, $3)', [vipId, proxyId, expiresAt || null]); // TODO-LINT: move to async function
...
router.delete('/proxies/:id', authenticateJWT, createRateLimit(15 * 60 * 1000, 20), async (req, res) => {
  try {
    await db.none('DELETE FROM vip_proxies WHERE id = $1', [req.params.id]); // TODO-LINT: move to async function
    res.json({ success: true });
  } catch (err) {
...
router.get('/metrics', authenticateJWT, createRateLimit(15 * 60 * 1000, 50), async (req, res) => {
  try {
    const vipCountRow = await db.one('SELECT COUNT(*) AS count FROM users WHERE is_vip = true'); // TODO-LINT: move to async function
    const ticketRow = await db.one('SELECT COUNT(*) AS count FROM support_tickets WHERE vip_priority_score > 0'); // TODO-LINT: move to async function
