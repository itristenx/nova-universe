...
router.post(
  '/mfa/challenge',
  mfaRateLimit,
  [body('tempSessionId').isUUID(), body('mfaMethod').isIn(['totp', 'sms', 'email', 'backup_code'])],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (mfaMethod === 'totp') {
        ...
        if (mfaConfig.rows.length === 0) {
          ...
          if (backupCodes.includes(code) && !usedCodes.includes(code)) {
            ...
            usedCodes.push(code);
            await db.query('UPDATE mfa_methods SET backup_codes_used = $1 WHERE id = $2', [
      ...
      await db.query('DELETE FROM mfa_challenges WHERE session_temp_id = $1', [tempSessionId]);
 ...
 *     tags: [Helix - Universal Login]
 */
router.post('/token/refresh', [body('refreshToken').isLength({ min: 1 })], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
...
async function checkRiskBasedMfaRequirement(userId, req) {
  try {
    ...
    const avgHour =
      userLogins.length > 0 ? userLogins.reduce((a, b) => a + b, 0) / userLogins.length : 12;
    ...
    if (parseInt(recentFailures.rows[0].count) > 3) {
      riskScore += 25;
      logger.info('Risk factor: Recent failures', {
        userId,
        failures: recentFailures.rows[0].count,
      });
    }
  ...
  try {
    ...
    const samlConfig = {
      ...
      callbackUrl:
        ...
        `${process.env.API_BASE_URL}/api/v1/helix/login/sso/callback/saml`,
      cert: config.metadata.cert,
      identifierFormat:
        config.metadata.identifierFormat ||
...
async function initiateOIDCLogin(config, state, redirectUrl) {
  try {
    const params = new URLSearchParams({
      ...
      redirect_uri:
        config.metadata.redirect_uri ||
        `${process.env.API_BASE_URL}/api/v1/helix/login/sso/callback/oidc`,
...
async function processSAMLResponse(requestBody) {
  try {
    ...
    const emailMatch = decodedResponse.match(
      /<saml:Attribute Name="http:\/\/schemas\.xmlsoap\.org\/ws\/2005\/05\/identity\/claims\/emailaddress"[^>]*>[\s\S]*?<saml:AttributeValue[^>]*>(.*?)<\/saml:AttributeValue>/i,
    );
    const nameMatch = decodedResponse.match(
      /<saml:Attribute Name="http:\/\/schemas\.xmlsoap\.org\/ws\/2005\/05\/identity\/claims\/name"[^>]*>[\s\S]*?<saml:AttributeValue[^>]*>(.*?)<\/saml:AttributeValue>/i,
    );
...
async function findOrCreateSSOUser(userProfile, tenantId) {
  try {
    ...
    const existingUser = await db.query('SELECT * FROM users WHERE email = $1 AND tenant_id = $2', [
      userProfile.email,
      tenantId,
    ]);
