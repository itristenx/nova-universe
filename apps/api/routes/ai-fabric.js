...
router.get('/models', authenticateJWT, async (req, res) => {
  try {
    ...
    try {
      if (
        aiFabric.internalProviders &&
        typeof aiFabric.internalProviders.getModelStats === 'function'
      ) {
...
router.get('/metrics/dashboard', authenticateJWT, async (req, res) => {
  try {
    ...
    const metrics = {
      ...
      // Performance grade
      performanceGrade:
        status.health === 'healthy' && dashboardData.metrics?.performance?.avgResponseTime < 1000
          ? 'A'
          : 'B',
