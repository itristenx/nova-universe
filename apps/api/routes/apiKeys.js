...
router.get('/', authenticateJWT, createRateLimit(15 * 60 * 1000, 30), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get API keys', errorCode: 'API_KEYS_ERROR' });
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        apiKey: { key, createdAt, description: req.body.description || '' },
      });
    } catch (err) {
      logger.error('Error creating API key:', err);
      res
        .status(500)
        .json({ success: false, error: 'Failed to create API key', errorCode: 'API_KEYS_ERROR' });
    }
  },
);
...
router.delete('/:key', authenticateJWT, createRateLimit(15 * 60 * 1000, 10), async (req, res) => {
  try {
    ...
    if (filtered.length === apiKeys.length) {
      return res
        .status(404)
        .json({ success: false, error: 'API key not found', errorCode: 'API_KEY_NOT_FOUND' });
    }
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to delete API key', errorCode: 'API_KEYS_ERROR' });
  }
});
