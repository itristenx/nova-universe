...
router.get('/config', async (req, res) => {
  try {
    ...
    if (!kioskId) {
      return res
        .status(400)
        .json({ success: false, error: 'Missing kiosk_id', errorCode: 'MISSING_KIOSK_ID' });
    }
    ...
    db.get('SELECT * FROM kiosks WHERE id = $1', [kioskId], (err, kiosk) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ success: false, error: 'Database error', errorCode: 'DB_ERROR' });
      }
      ...
      db.all('SELECT key, value FROM config', (cErr, rows) => {
        if (cErr) {
          ...
          return res
            .status(500)
            .json({ success: false, error: 'Config error', errorCode: 'CONFIG_ERROR' });
        }
    ...
    res
      .status(500)
      .json({ success: false, error: 'Internal server error', errorCode: 'INTERNAL_ERROR' });
  }
});
