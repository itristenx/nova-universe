...
async function setupDefaultAdmin() {
  try {
    ...
    const existingAdmin = await db.query(
      'SELECT id FROM users WHERE email = $1 OR "isDefault" = true LIMIT 1',
      [adminEmail]
    );
    ...
    if (existingAdmin.rows.length === 0) {
      ...
      const result = await db.query(
        'INSERT INTO users (id, name, email, passwordHash, isDefault) VALUES ($1, $2, $3, $4, true) RETURNING id',
        [newUserId, 'Admin', 'admin@example.com', adminHash]
      );
