...
export class NovaSentinelIntegration extends EventEmitter {
    ...
    monitoringInterval;
    isInitialized = false;
    sentinelApiUrl = process.env.NOVA_SENTINEL_API_URL || 'http://localhost:3001/api';
    novaCoreApiUrl = process.env.NOVA_API_BASE_URL || 'http://localhost:3000/api';
    apiKey = process.env.NOVA_SENTINEL_API_KEY || '';
     ...
     */
    async registerAIMonitor(monitor) {
     ...
     */
    async updateMonitorStatus(monitorId, status, metrics) {
    ...
    getDashboardData() {
        ...
        const recentAlerts = alerts.filter((a) => a.timestamp >= new Date(Date.now() - 24 * 60 * 60 * 1000) && !a.resolved);
        return {
            overview: {
    ...
    // Private methods
    async testSentinelConnectivity() {
        try {
     ...
     */
    async registerAIComponentMonitors() {
        const aiComponents = [
            {
                name: 'AI Fabric Core',
                type: 'http',
    ...
    async registerDefaultAIMonitors() {
    ...
    async handleStatusChange(monitor, previousStatus, newStatus) {
    ...
    async resolveComponentIncidents(component, resolution) {
        const componentIncidents = Array.from(this.incidents.values()).filter((i) => i.component === component && i.status !== 'resolved');
        for (const incident of componentIncidents) {
            await this.resolveIncident(incident.id, resolution);
        }
    }
    async generateAIAnalysis(incident) {
        if (!aiFabric) {
            return { confidence: 0 };
        }
        try {
    ...
    async handleProviderHealthChange(event) {
        const monitorId = `provider-${event.providerId}`;
        const monitor = Array.from(this.monitors.values()).find((m) => m.providerId === event.providerId);
        if (monitor) {
    ...
    async handleAIRequestFailure(event) {
        const alert = {
    ...
    async handleMetricRecorded(event) {
    ...
    async handleCreateIncident(args) {
        try {
    ...
    async handleGetStatusPage(args) {
        try {
    ...
    async handleReportAIError(args) {
        try {
    ...
    async createSentinelMonitor(monitor) {
        try {
    ...
    async checkExistingMonitor(name) {
        try {
            const response = await axios.get(`${this.sentinelApiUrl}/monitors`, {
                headers: this.getAuthHeaders(),
                params: { name },
    ...
    async sendMetricToSentinel(metric) {
        try {
            ...
            if (metric.metricType === 'performance') {
                ...
                const monitor = Array.from(this.monitors.values()).find((m) => m.component === metric.providerId);
                if (monitor) {
                    await this.updateMonitorStatus(monitor.id, metric.value > this.config.escalationThresholds.aiResponseTime ? 'down' : 'up', { responseTime: metric.value });
    ...
    async createSentinelIncidentFromAlert(alert) {
        try {
            await this.createIncident({
    ...
    async updateMonitorStatusFromEvent(event) {
        try {
    ...
    async updateProviderMonitorStatus(event) {
        try {
            const monitor = Array.from(this.monitors.values()).find((m) => m.component === event.providerId);
            if (monitor) {
                await this.updateMonitorStatus(monitor.id, event.healthy ? 'up' : 'down', {
    ...
    async reportAIFailureToSentinel(event) {
        try {
            await this.createIncident({
    ...
    async updateSentinelMonitor(monitor) {
        try {
    ...
    async createSentinelIncident(incident) {
        try {
    ...
    async updateSentinelIncident(incident) {
        try {
    ...
    async syncMonitorsFromSentinel() {
        try {
            const response = await axios.get(`${this.sentinelApiUrl}/monitors`, {
                headers: this.getAuthHeaders(),
                params: { tags: 'ai-fabric' },
                ...
                for (const sentinelMonitor of response.data.monitors) {
                    ...
                    if (this.monitors.has(sentinelMonitor.id)) {
                        ...
                        if (localMonitor) {
                            ...
                            logger.debug(`Synced monitor from Sentinel: ${sentinelMonitor.id} (${localMonitor.status})`);
                        }
                    }
                }
            }
            ...
            logger.warn(`Failed to sync monitors from Sentinel: ${error.message}`);
        }
    }
    async escalateToGoAlert(incident) {
        // This will be implemented in the GoAlert integration
        logger.info(`Would escalate to GoAlert - incident: ${incident.id}`);
    }
    async escalateSecurityAlert(alert) {
        // This will be implemented in the GoAlert integration
    ...
    async createIncidentTicket(incident) {
        try {
            ...
            await db.query?.('INSERT INTO tickets (id, ticket_id, title, description, priority, status, requested_by_id, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)', [id, `INC-${Date.now()}`, title, description, incident.severity, 'open', null, now, now]);
            return id;
        }
        catch (error) {
            logger.warn(`Ticket creation fallback (db unavailable): ${error?.message}`);
            return crypto.randomUUID();
        }
    }
    async resolveIncidentTicket(ticketId, resolution) {
        try {
            const { default: db } = await import('../db.js');
            await db.query?.('UPDATE tickets SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', ['resolved', ticketId]);
            if (resolution) {
                await db.query?.('INSERT INTO ticket_comments (id, ticket_id, content, type, created_at) VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)', [crypto.randomUUID(), ticketId, `Resolution: ${resolution}`, 'internal']);
            }
        }
        catch (error) {
            logger.warn(`Ticket resolution fallback (db unavailable) - ticket: ${ticketId}, error: ${error?.message}`);
        }
    }
    getAuthHeaders() {
        const headers = {
