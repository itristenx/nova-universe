...
class S3StorageProvider extends StorageProvider {
  ...
  async _doInitializeS3Client() {
    try {
      ...
      const { PutObjectCommand, GetObjectCommand, DeleteObjectCommand, HeadObjectCommand } = await import('@aws-sdk/client-s3')
      ...
      throw new Error('S3 storage not available. Install @aws-sdk/client-s3 and @aws-sdk/s3-request-presigner packages.')
    }
  }
...
class FileStorageManager {
  ...
  initializeProviders(config) {
    ...
    this.providers.set('local', new LocalStorageProvider((config.local && config.local.config) || {
      basePath: process.env.LOCAL_STORAGE_BASE_PATH || path.join(process.cwd(), 'apps/api/uploads'),
    ...
    if (this.needsS3Provider()) {
      try {
        this.providers.set('s3', new S3StorageProvider((config.s3 && config.s3.config) || {
  ...
  needsS3Provider() {
    ...
    return Object.values(this.hybridRules).some(rule => rule.provider === 's3') ||
  ...
  determineStorageProvider(filename, fileType, context = 'default') {
    ...
    if (defaultRule.provider === 's3' && this.s3Functional && this.providers.has('s3')) {
      return 's3'
    } else if (defaultRule.provider === 's3' && defaultRule.fallback && this.providers.has(defaultRule.fallback)) {
