...
export class StatusPageService {
    cacheDir = '/tmp/nova-status-cache';
    cache = new Map();
    cacheExpiry = new Map();
    constructor() {
     ...
     */
    async getStatusPage(slugOrDomain) {
        // Check cache first
...
<body style="${statusPage.background_color ? `background-color: ${statusPage.background_color};` : ''} 
    ...
    ${incidents.filter((i) => i.status !== 'resolved').length > 0
            ? this.generateIncidentsHTML(incidents.filter((i) => i.status !== 'resolved'), 'Active Incidents')
            : ''}
    ...
    <!-- Incident History -->
    ${incidents.filter((i) => i.status === 'resolved').length > 0
            ? this.generateIncidentsHTML(incidents.filter((i) => i.status === 'resolved'), 'Recent Incidents')
            : ''}
     ...
     */
    generateAppleInspiredCSS(statusPage) {
        return `
        :root {
    ...
    async queryStatusPageBySlugOrDomain(slugOrDomain) {
        try {
            const { default: db } = await import('../db.js');
            const res = await db.query?.(`SELECT * FROM nova_status_pages WHERE slug = $1 OR domain_name = $1 LIMIT 1`, [slugOrDomain]);
    ...
    async calculateUptime(statusPageId, days) {
        try {
            const { default: db } = await import('../db.js');
            const res = await db.query?.(`SELECT AVG(CASE WHEN is_up THEN 100.0 ELSE 0.0 END) AS uptime
         FROM nova_monitor_summary sms
         JOIN nova_status_page_monitors spm ON spm.monitor_id = sms.id
         WHERE spm.status_page_id = $1
           AND sms.last_check_time >= NOW() - ($2 || ' days')::INTERVAL`, [statusPageId, days]);
    ...
    generateMonitorGroupHTML(title, monitors, status) {
        return `
    ...
    generateIncidentsHTML(incidents, title) {
        return `
     ...
     */
    async generateStatusBadge(badge, monitorStatus, uptime) {
        const colors = {
            up: badge.color_up || '#4c1',
            down: badge.color_down || '#e05d44',
            pending: badge.color_pending || '#dfb317',
        };
        const color = monitorStatus === 'up' ? colors.up : monitorStatus === 'down' ? colors.down : colors.pending;
