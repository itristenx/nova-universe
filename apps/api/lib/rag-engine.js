...
export class NovaRAGEngine extends EventEmitter {
     ...
     */
    async queryWithRBAC(ragQuery, startTime) {
     ...
     */
    async queryWithoutRBAC(ragQuery, startTime) {
        // Expand query if enabled
        if (ragQuery.options.expandQuery) {
            ragQuery.query = await this.expandQuery(ragQuery.query);
        }
     ...
     */
    async filterChunksWithRBAC(chunks, userId, tenantId, action = 'read') {
     ...
     */
    async addDocuments(documents, rbacContext) {
        try {
     ...
     */
    async processDocumentWithRBAC(doc, rbacContext) {
     ...
     */
    async setDocumentRBACPermissions(documentId, metadata, rbacContext) {
        try {
            const permission = {
                documentId,
    ...
    async removeDocument(documentId) {
        try {
            ...
            const chunksToRemove = Array.from(this.documentChunks.values()).filter((chunk) => chunk.documentId === documentId);
    ...
    async initializeVectorStore(store) {
        switch (store.type) {
            case 'chromadb':
    ...
    async addToChromaDB(chunk, store) {
    ...
    async addToPinecone(chunk, store) {
    ...
    async addToLocalStore(chunk, store) {
    ...
    async removeFromVectorStore(chunkId) {
        const store = this.vectorStores.get(this.config.defaultVectorStore);
        if (!store)
            return;
    ...
    async removeFromChromaDB(chunkId, store) {
    ...
    async removeFromPinecone(chunkId, store) {
    ...
    async removeFromLocalStore(chunkId, store) {
    ...
    async semanticSearch(query, queryEmbedding) {
    ...
    async hybridSearch(query, queryEmbedding) {
    ...
    mergeSearchResults(semanticResults, keywordResults) {
        ...
        return Array.from(merged.values()).sort((a, b) => (b.metadata.relevanceScore || 0) - (a.metadata.relevanceScore || 0));
    }
    ...
    async generateContextSummary(query, chunks) {
     ...
     */
    applyNovaDataPrioritization(chunk, similarity) {
    ...
    async updateKnowledgeGraph(doc) {
