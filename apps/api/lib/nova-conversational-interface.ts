...
import { novaAIAgent, ConversationMessage, RichMessageContent, MessageAction } from './nova-ai-agent-framework.js';
...
export interface ChannelCapabilities {
  id: string;
  name: string;
  type: 'web' | 'mobile' | 'email' | 'slack' | 'teams' | 'api';
  features: {
...
export interface ConversationSession {
  ...
  preferences: {
    ...
    messageFormat: 'html' | 'markdown' | 'plain';
...
export interface MessageTemplate {
  id: string;
  name: string;
  category: 'greeting' | 'escalation' | 'resolution' | 'error' | 'information';
  content: string;
...
export interface ConversationSuggestion {
  id: string;
  type: 'intent' | 'response' | 'action' | 'escalation';
  text: string;
  confidence: number;
  category: string;
...
export class NovaConversationalInterface extends EventEmitter {
  private channels: Map<string, ChannelCapabilities> = new Map();
  private sessions: Map<string, ConversationSession> = new Map();
  private messageTemplates: Map<string, MessageTemplate> = new Map();
  private activeTyping: Map<string, Date> = new Map();
  ...
  async processMessage(
    ...
    try {
      ...
      const adaptedMessages = await this.adaptResponseForChannel(
   ...
   */
  private async adaptResponseForChannel(
    messages: ConversationMessage[],
    channel: ChannelCapabilities
  ): Promise<ConversationMessage[]> {
    const adaptedMessages: ConversationMessage[] = [];
    ...
    for (const message of messages) {
      ...
      if (!channel.features.richMessages && adapted.richContent) {
        ...
        if (adapted.richContent.type === 'buttons' || adapted.richContent.type === 'quick_replies') {
          const options = adapted.richContent.items?.map((item: any, index: number) => 
          ...
          const fields = adapted.richContent.items?.map((item: any) => 
   ...
   */
  private async checkRateLimit(session: ConversationSession): Promise<void> {
   ...
   */
  private startTyping(sessionId: string): void {
   ...
   */
  private startPresenceManager(): void {
    setInterval(() => {
      const now = new Date();
        ...
        for (const [key, value] of Object.entries(obj)) {
          if (typeof value === 'string') {
            obj[key] = value.replace(/{{(\w+)}}/g, (match, varName) => 
              String(variables[varName] || match)
            );
   ...
   */
  getChannelCapabilities(channelId: string): ChannelCapabilities | undefined {
   ...
   */
  getSupportedChannels(): ChannelCapabilities[] {
   ...
   * Get active sessions
   */
  getActiveSessions(userId?: string, tenantId?: string): ConversationSession[] {
