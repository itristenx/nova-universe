...
export class ExtendedMonitorService {
    ...
    async checkDns(check) {
        ...
        try {
            ...
            if (expected_address) {
                const addressMatches = result.addresses.some((addr) => typeof addr === 'string' ? addr === expected_address : addr.address === expected_address);
     ...
     */
    async checkSslCertificate(check) {
