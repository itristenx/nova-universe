...
export class NovaRAGRBAC extends EventEmitter {
     ...
     */
    async filterDocuments(documents, userId, tenantId, action = 'read') {
     ...
     */
    async checkDocumentAccess(documentId, userId, tenantId, action = 'read') {
     ...
     */
    async setDocumentPermission(permission) {
    ...
    // Private methods
    async evaluatePolicies(user, context) {
        const applicablePolicies = Array.from(this.policies.values()).filter(policy => policy.isActive &&
    ...
    policyApplies(policy, user, context) {
        // Check if policy applies to the resource
        if (policy.resources.length > 0) {
            const resourceMatches = policy.resources.some(resource => this.resourceMatches(resource, context.resource));
            if (!resourceMatches)
                return false;
        }
    ...
    async evaluatePolicy(policy, user, context) {
    ...
    async evaluateCondition(condition, user, context) {
        ...
        switch (condition.type) {
            case 'attribute':
                actualValue = user.attributes[condition.field] || user[condition.field];
                break;
            case 'time':
    ...
    evaluateExpression(expression, user, context) {
        ...
        try {
            ...
            const processed = expression
                ...
                const value = user[field] || user.attributes[field];
    ...
    createAllowDecision(reason, context, policyId) {
        return {
            granted: true,
            reason,
            policyId,
    ...
    createDenyDecision(reason, context) {
        return {
            granted: false,
            reason,
            auditLog: {
    ...
    async logAccess(context, decision) {
