...
class NotificationService {
     ...
     */
    async getRelevantSubscriptions(event) {
        try {
            const { default: db } = await import('../db.js');
            const conditions = ['tenant_id = $1', 'verified = true'];
            const params = [event.tenant_id];
            let idx = 2;
            if (event.monitor_id) {
     ...
     */
    async sendToSubscription(subscription, event) {
     ...
     */
    async sendEmail(email, event, channel) {
     ...
     */
    async sendSMS(phone, event, channel) {
     ...
     */
    async sendWebhook(url, event, channel) {
        const payload = {
     ...
     */
    async sendSlack(webhookUrl, event, channel) {
     ...
     */
    async sendDiscord(webhookUrl, event, channel) {
 ...
 */
export async function sendIncidentNotification(incident, type) {
    const event = {
        type: `incident_${type}`,
 ...
 */
export async function sendMonitorNotification(monitor, status) {
    const event = {
