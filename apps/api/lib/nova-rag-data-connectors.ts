...
export interface DataSourceConfig {
  id: string;
  name: string;
  type: 'knowledge_base' | 'tickets' | 'service_catalog' | 'documentation' | 'monitoring' | 'external';
  enabled: boolean;
...
export class NovaRAGDataConnectors extends EventEmitter {
   ...
   */
  async syncDataSource(connectorId: string): Promise<SyncResult> {
   ...
   */
  private async syncKnowledgeBase(connector: DataSourceConnector): Promise<SyncResult> {
    const startTime = Date.now();
   ...
   */
  private async syncTickets(connector: DataSourceConnector): Promise<SyncResult> {
    const startTime = Date.now();
    const result: SyncResult = {
      sourceId: connector.id,
      documentsProcessed: 0,
   ...
   */
  private async syncServiceCatalog(connector: DataSourceConnector): Promise<SyncResult> {
    const startTime = Date.now();
    const result: SyncResult = {
      sourceId: connector.id,
      documentsProcessed: 0,
   ...
   */
  private async syncMonitoringData(connector: DataSourceConnector): Promise<SyncResult> {
    const startTime = Date.now();
    const result: SyncResult = {
      sourceId: connector.id,
      documentsProcessed: 0,
   ...
   */
  private async syncGoAlertData(result: SyncResult): Promise<void> {
    try {
   ...
   */
  private async syncSentinelData(result: SyncResult): Promise<void> {
    try {
   ...
   */
  private async syncDocumentation(connector: DataSourceConnector): Promise<SyncResult> {
   ...
   */
  getConnectorStatus(): DataSourceConnector[] {
  ...
  private async startSyncLoop(connector: DataSourceConnector): Promise<void> {
  ...
  private stopSyncLoop(connectorId: string): void {
  ...
  private prepareKBContent(article: any): string {
...
Last Updated: ${new Date(article.updated_at).toLocaleDateString()}`;
  ...
  private prepareTicketContent(ticket: any): string {
...
${ticket.resolved_at ? `Resolved: ${new Date(ticket.resolved_at).toLocaleDateString()}` : ''}`;
  ...
  private prepareServiceItemContent(item: any): string {
...
Last Updated: ${new Date(item.updated_at).toLocaleDateString()}`;
  ...
  private getTicketAccessLevel(ticket: any): string {
  ...
  private getDepartmentFromTicket(ticket: any): string {
