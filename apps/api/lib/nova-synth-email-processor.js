...
export class NovaSynthEmailProcessor extends EventEmitter {
     ...
     */
    async processEmailForTicket(email, emailAccount, context) {
        ...
        try {
            ...
            const [customerAnalysis, incidentAnalysis, priorityAnalysis, contentAnalysis, automationAnalysis] = await Promise.all([
            ...
            const assignmentAnalysis = await this.analyzeAssignment(incidentAnalysis, customerAnalysis, context);
            ...
            const analysis = {
                ...
                recommendations: await this.generateRecommendations(customerAnalysis, incidentAnalysis, priorityAnalysis, assignmentAnalysis, contentAnalysis, emailContent),
                metadata: {
            ...
            logger.info(`Email ${emailId} processed successfully in ${analysis.metadata.processingTime}ms ` +
     ...
     */
    async analyzeCustomer(email, emailAccount, context) {
        try {
     ...
     */
    async analyzeIncident(emailContent, context) {
        try {
      ...
      `;
            ...
            try {
                ...
                const classifier = experiments.find(exp => exp.modelName.includes('itsm') || exp.modelName.includes('incident'));
                if (classifier) {
                    mlClassification = await novaMLPipeline.predictWithCosmoPersonality(classifier.id, emailContent.subject + ' ' + emailContent.body, { context: 'incident_classification' });
                }
            }
            catch (error) {
            ...
            if (context?.systemContext) {
                incidentInfo.affectedSystems = this.matchAffectedSystems(emailContent, context.systemContext.knownSystems);
            }
            return incidentInfo;
        }
        catch (error) {
     ...
     */
    async analyzePriority(emailContent, context) {
        try {
     ...
     */
    async analyzeContent(emailContent, context) {
        try {
     ...
     */
    async analyzeAutomation(emailContent, context) {
        try {
      ...
      `;
            ...
            if (context) {
                ...
                if (context.systemContext?.commonIssues?.length) {
                    automationQuery += `\n\nCommon Issues: ${context.systemContext.commonIssues.map((issue) => issue.description || issue).join(', ')}`;
                }
     ...
     */
    async analyzeAssignment(incidentAnalysis, customerAnalysis, context) {
        try {
     ...
     */
    async generateRecommendations(customerAnalysis, incidentAnalysis, priorityAnalysis, assignmentAnalysis, contentAnalysis, emailContent) {
        return {
            ticketData: {
                ...
                customFields: this.generateCustomFields(customerAnalysis, incidentAnalysis, priorityAnalysis, contentAnalysis),
            },
            assignment: {
    ...
    extractCustomerInfoFromRAG(ragResponse, email, name) {
    ...
    extractIncidentInfoFromRAG(ragResponse, mlClassification, emailContent) {
    ...
    extractPriorityInfoFromRAG(ragResponse, emailContent) {
    ...
    extractKeywordsFromRAG(ragResponse, emailContent) {
        ...
        if (ragResponse.ragContext?.sources) {
            ragResponse.ragContext.sources.forEach((source) => {
                if (source.title) {
    ...
    extractAutomationInfoFromRAG(ragResponse) {
        return {
    ...
    extractAssignmentInfoFromRAG(ragResponse, incidentAnalysis) {
        return {
    ...
    async fallbackBasicProcessing(email, emailAccount) {
        const emailContent = this.extractEmailContent(email);
    ...
    fallbackIncidentAnalysis(emailContent) {
        ...
        return {
            ...
            impactAssessment: {
                scope: severity === 'high' ? 'multiple' : 'individual',
                severity,
    ...
    fallbackPriorityAnalysis(emailContent) {
    ...
    fallbackContentAnalysis(emailContent) {
        return {
    ...
    async initializeCustomerEnrichment() {
        // Initialize customer enrichment data
        logger.debug('Initializing customer enrichment data');
    }
    extractSystemsFromContent(content) {
    ...
    extractBusinessContext(emailContent) {
        const subject = (emailContent.subject || '').toLowerCase();
        const body = (emailContent.body || '').toLowerCase();
        const fullText = subject + ' ' + body;
    ...
    mapIncidentTypeToTicketType(incidentType) {
        const mapping = {
            'incident': 'INCIDENT',
    ...
    generateEnhancedTags(incidentAnalysis, contentAnalysis) {
    ...
    generateCustomFields(customerAnalysis, incidentAnalysis, priorityAnalysis, contentAnalysis) {
        return {
            ai_processed: true,
    ...
    shouldSendAutoReply(priorityAnalysis, customerAnalysis) {
    ...
    recommendEmailTemplate(incidentAnalysis, priorityAnalysis) {
        if (priorityAnalysis.level === 'CRITICAL') {
    ...
    getAdditionalRecipients(customerAnalysis, incidentAnalysis) {
        const recipients = [];
    ...
    getEscalationNotifications(priorityAnalysis, assignmentAnalysis) {
        const notifications = [];
        ...
        if (assignmentAnalysis.skillsRequired?.length > 0) {
            assignmentAnalysis.skillsRequired.forEach((skill) => {
    ...
    calculateSLARequirements(priorityAnalysis, customerAnalysis, incidentAnalysis) {
        const slaMapping = {
     ...
     */
    async testEmailProcessing(testEmail, testContext) {
