...
export class AdvancedFeaturesService {
    ...
    async assignTagsToMonitor(monitorId, tagIds) {
    ...
    async getScheduledMaintenanceWindows(daysAhead = 7) {
        const now = new Date();
    ...
    async scheduleMaintenanceNotifications(window) {
    ...
    async sendMaintenanceNotification(window, timing) {
     ...
     */
    async createProxyConfiguration(proxyData) {
        const proxy = {
            id: this.generateId(),
            name: proxyData.name,
    ...
    async getProxyConfiguration(proxyId) {
    ...
    async getCertificateExpiringSoon(days = 30) {
     ...
     * Ping Chart Data
     */
    async generatePingChartData(monitorId, hours = 24) {
    ...
    async queryTagsByTenant(tenantId) {
        const db = (await import('../db.js')).default;
        const res = tenantId
    ...
    async assignMonitorTags(monitorId, tagIds) {
        const db = (await import('../db.js')).default;
    ...
    async queryMonitorsByTag(tagId) {
        const db = (await import('../db.js')).default;
        const res = await db.query('SELECT monitor_id FROM monitor_tags WHERE tag_id = $1', [tagId]);
        return (res.rows || []).map((r) => r.monitor_id);
    }
    async saveMaintenanceWindow(window) {
        const db = (await import('../db.js')).default;
        await db.query(`
    ...
    async saveProxyConfiguration(proxy) {
        const db = (await import('../db.js')).default;
        await db.query(`
    ...
    async queryProxyConfiguration(proxyId) {
        const db = (await import('../db.js')).default;
    ...
    async queryCertificatesExpiringSoon(days) {
        const db = (await import('../db.js')).default;
    ...
    async queryMonitorResults(monitorId, hours) {
        const db = (await import('../db.js')).default;
        const res = await db.query(`
