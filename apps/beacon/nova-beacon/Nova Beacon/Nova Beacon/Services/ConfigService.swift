...
class ConfigService: ObservableObject {
    ...
    func load() async {
        ...
        guard let url = URL(string: "\(APIConfig.baseURL)/api/v2/beacon/config") else {
        ...
        if let kioskId = UserDefaults.standard.string(forKey: "kioskId"),
           let kurl = URL(string: "\(APIConfig.baseURL)/api/v2/beacon/kiosks/\(kioskId)") {
    ...
    func verifyPassword(_ password: String, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "\(APIConfig.baseURL)/api/v2/beacon/verify-password") else {
    ...
    func verifyPin(_ pin: String, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "\(APIConfig.baseURL)/api/v2/beacon/verify-admin-pin") else {
