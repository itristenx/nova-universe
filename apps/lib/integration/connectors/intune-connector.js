...
export class IntuneConnector extends IConnector {
  ...
  async health() {
    try {
      ...
      const overallStatus = devicesResponse.status === 200 && appsStatus === 'healthy' && complianceStatus === 'healthy'
      ...
      return {
        ...
        issues: overallStatus !== HealthStatus.HEALTHY ? ['Some Intune API endpoints degraded'] : []
      };
    } catch (error) {
      return {
  ...
  async remoteWipe(deviceId, parameters = {}) {
    try {
      ...
      await this.client.post(`/deviceManagement/managedDevices/${deviceId}/wipe`, {
        keepEnrollmentData,
        keepUserData
      }, {
        headers: { 'Authorization': `Bearer ${this.accessToken}` }
