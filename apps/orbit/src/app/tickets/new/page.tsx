...
export default function NewTicketPage() {
  ...
  return (
    <main className="mx-auto max-w-xl p-8">
      <h1 className="mb-4 text-2xl font-bold">Submit a New Ticket</h1>
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        {subcategories.length > 0 && (
          <Select
            ...
            onValueChange={(sub) => setForm((f) => ({ ...f, subcategory: sub }))}
          >
            <SelectTrigger className="w-full">
        ...
        <Select
          value={form.priority}
          onValueChange={(p) =>
            setForm((f) => ({ ...f, priority: p as 'low' | 'medium' | 'high' | 'critical' }))
          }
        >
          <SelectTrigger className="w-full">
        ...
        {success && <div className="text-success mt-2 text-sm">Ticket submitted!</div>}
      </form>
    </main>
  );
}
