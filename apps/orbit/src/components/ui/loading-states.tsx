...
export function LoadingOverlay({
  ...
  return (
    <div className={cn('relative', className)}>
      {children}
      <div
        className="absolute inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-sm dark:bg-gray-900/80"
        role="dialog"
        aria-modal="true"
        aria-label={message}
      >
...
export function CardSkeleton({ className = '' }: CardSkeletonProps) {
  return (
    <div
      className={cn(
        'space-y-4 rounded-lg border border-gray-200 p-6 dark:border-gray-700',
        className,
      )}
    >
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="rounded-lg border border-gray-200 p-6 dark:border-gray-700">
        <LoadingSkeleton variant="text" className="mb-4 h-6 w-1/4" />
        <TableSkeleton />
      </div>
    </div>
  );
