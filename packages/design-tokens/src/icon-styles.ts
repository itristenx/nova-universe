...
export function generateIconCSS(): string {
  ...
  cssRules.push(
    '.nova-icon-notification { width: 1.25rem; height: 1.25rem; margin-right: 0.75rem; flex-shrink: 0; }',
  );

  // Interactive states
  cssRules.push(
    '.nova-icon-interactive { cursor: pointer; transition: color 0.2s ease, opacity 0.2s ease; }',
  );
  ...
  cssRules.push(
    '.nova-icon-pulse { animation: nova-icon-pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite; }',
  );
  cssRules.push('.nova-icon-bounce { animation: nova-icon-bounce 1s infinite; }');

  // Keyframes
  cssRules.push(
    '@keyframes nova-icon-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }',
  );
  cssRules.push('@keyframes nova-icon-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }');
  cssRules.push(
    '@keyframes nova-icon-bounce { 0%, 100% { transform: translateY(-25%); animation-timing-function: cubic-bezier(0.8, 0, 1, 1); } 50% { transform: none; animation-timing-function: cubic-bezier(0, 0, 0.2, 1); } }',
  );
