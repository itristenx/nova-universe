...
`;
  ...
  async migrateFromSQLite(sqliteDbPath) {
    ...
    const db = new sqlite3.default.Database(sqliteDbPath);

    try {
      // Get all tables from SQLite
      const tables = await this.getSQLiteTables(db);
      
      for (const table of tables) {
        if (table === 'schema_migrations') continue; // Skip migration table
        
        await this.migrateTable(db, table);
      }

      logger.info('✅ Data migration from SQLite completed successfully');
    } catch (error) {
  ...
  async migrateTable(sqliteDb, tableName) {
    ...
    if (data.length === 0) {
      logger.info(`ℹ️  Table ${tableName} is empty, skipping`);
      return;
    }
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      await postgresManager.transaction(async (client) => {
        for (const row of batch) {
          ...
          await postgresManager.query(insertQuery, values, { client });
          inserted++;
        }
      });
    }

    logger.info(`✅ Migrated ${inserted} records from ${tableName}`);
  }
