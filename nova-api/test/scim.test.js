...
describe('SCIM user management', function() {
  it('creates a user', async function() {
    ...
    const row = await new Promise((resolve) => {
      dbWrapper.get('SELECT * FROM users WHERE id=?', [res.body.id], (err, r) => resolve(r));
    });
  ...
  it('updates a user', async function() {
    const id = await new Promise((resolve) => {
      dbWrapper.run('INSERT INTO users (name, email) VALUES (?, ?)', ['Old', 'old@e.com'], function(err) {
        resolve(this.lastID);
      });
    });
    ...
    const row = await new Promise((resolve) => {
      dbWrapper.get('SELECT * FROM users WHERE id=?', [id], (err, r) => resolve(r));
    });
