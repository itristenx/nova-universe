      ...
      `, [userId], (err, ticketStats) => {
        ...
        db.get(`
          SELECT 
            ...
            AVG(JULIANDAY(resolved_at) - JULIANDAY(created_at)) * 24 * 60 as avg_resolution_time,
  ...
  async (req, res) => {
    try {
      ...
      let query = `
        ...
        WHERE t.assigned_to_id = ? AND t.deleted_at IS NULL
      `;
      const params = [userId];
...
router.put('/tickets/:ticketId/update',
  ...
  async (req, res) => {
    try {
      ...
      db.get('SELECT * FROM tickets WHERE ticket_id = ? AND assigned_to_id = ? AND deleted_at IS NULL', 
        ...
        if (timeSpent) {
          updates.push('actual_time_minutes = COALESCE(actual_time_minutes, 0) + ?');
        ...
        if (updates.length > 0) {
          ...
          db.run(updateQuery, params, (updateErr) => {
            ...
            db.run(
              'INSERT INTO ticket_logs (id, ticket_id, user_id, action, details, timestamp) VALUES (?, ?, ?, ?, ?, ?)',
...
router.post('/tickets/:ticketId/claim',
  ...
  async (req, res) => {
    try {
      ...
      db.get('SELECT * FROM tickets WHERE ticket_id = ? AND deleted_at IS NULL', [ticketId], (err, ticket) => {
        if (err) {
          logger.error('Error checking ticket:', err);
          return res.status(500).json({
            success: false,
        ...
        db.run(
          'UPDATE tickets SET assigned_to_id = ?, status = ?, updated_at = ? WHERE id = ?',
          ...
          (updateErr) => {
            ...
            // Log the activity
            db.run(
              'INSERT INTO ticket_logs (id, ticket_id, user_id, action, timestamp) VALUES (?, ?, ?, ?, ?)',
  ...
  async (req, res) => {
    try {
      ...
      `, [userId, start, end], (err, entries) => {
        ...
        const totalMinutes = (entries || []).reduce((sum, entry) => sum + (entry.actual_time_minutes || 0), 0);
        const totalHours = totalMinutes / 60;
        const ticketsWorked = entries ? entries.length : 0;
        ...
        const timesheet = {
          ...
          entries: (entries || []).map(entry => ({
            ...
            title: entry.title,
            timeSpent: entry.actual_time_minutes,
      ...
      logger.error('Error in timesheet endpoint:', error);
      res.status(500).json({
        success: false,
        error: 'Failed to fetch timesheet',
        errorCode: 'TIMESHEET_ERROR'
