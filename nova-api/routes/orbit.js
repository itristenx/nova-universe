  ...
  async (req, res) => {
    try {
      ...
      let query = `
        ...
        WHERE t.requested_by_id = ? AND t.deleted_at IS NULL
      `;
      const params = [userId];
    ...
    try {
      ...
      db.get('SELECT MAX(CAST(SUBSTR(ticket_id, 5) AS INTEGER)) as max_id FROM tickets', [], (err, row) => {
        ...
        db.run(insertQuery, [
          ...
          if (insertErr) {
            logger.error('Error creating ticket:', insertErr);
            return res.status(500).json({
              success: false,
              error: 'Failed to create ticket',
          ...
          if (attachments.length > 0) {
            const attachmentPromises = attachments.map((attachment, index) => {
              return new Promise((resolve, reject) => {
                db.run(
                  'INSERT INTO ticket_attachments (id, ticket_id, file_path, file_name, uploaded_by_id, uploaded_at) VALUES (?, ?, ?, ?, ?, ?)',
                  [
                    require('uuid').v4(),
                    ticketId,
                    attachment,
  ...
  async (req, res) => {
    try {
      ...
      db.get(`
        ...
        FROM tickets t
        LEFT JOIN users a ON t.assigned_to_id = a.id
        WHERE t.ticket_id = ? AND t.requested_by_id = ? AND t.deleted_at IS NULL
...
router.post('/feedback',
  ...
  async (req, res) => {
    try {
      ...
      db.run(
        'INSERT INTO feedback (name, message, user_id, feedback_type, timestamp) VALUES (?, ?, ?, ?, ?)',
