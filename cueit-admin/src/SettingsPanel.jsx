...
export default function SettingsPanel({ open, onClose, config, setConfig, saveConfig }) {
  ...
  const toggleKiosk = async (id, active) => {
      ...
      >
        ...
        <div className="mb-4 flex border-b border-gray-700">
          <button
            ...
            className={`mr-4 pb-2 ${tab === 'general' ? 'border-b-2 border-blue-500' : ''}`}
          >
            General
          </button>
          <button
            onClick={() => setTab('kiosks')}
            className={`pb-2 ${tab === 'kiosks' ? 'border-b-2 border-blue-500' : ''}`}
          >
            Kiosks
          </button>
        </div>
              ...
              <tbody>
                {kiosks.map((k) => (
                  <tr key={k.id} className="border-t border-gray-700">
                    ...
                    <td className="py-1">
                      <button
                        onClick={() => toggleKiosk(k.id, k.active)}
                    ...
                    <td className="py-1 pl-2">
                      <button onClick={() => saveKiosk(k)} className="px-2 py-1 rounded text-xs bg-blue-600">
                        Save
                      </button>
                    </td>
                  </tr>
