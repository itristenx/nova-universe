...
describe('Inventory Enhancement Implementation', () => {
  ...
  describe('Metadata Collection', () => {
    ...
    it('should handle inactive kiosks properly', async () => {
      ...
      await expect(inventoryService.collectKioskMetadata('kiosk-001')).rejects.toThrow(
        'Kiosk kiosk-001 is not active',
      );
    });
  ...
  describe('Error Handling', () => {
    it('should handle database connection errors gracefully', async () => {
      ...
      await expect(inventoryService.syncAssetWithKiosk(1, 'kiosk-001')).rejects.toThrow(
    ...
    it('should handle missing assets appropriately', async () => {
      ...
      await expect(inventoryService.syncAssetWithKiosk(999, 'kiosk-001')).rejects.toThrow(
        'Asset not found',
      );
    });
...
TEST002,SN987654321,HP ProDesk 400,Finance,active`;
      ...
      const result = await inventoryService.importAssets(csvData, 'test-import.csv', 'test-user');
