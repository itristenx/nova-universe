...
describe('File Storage System', () => {
  ...
  describe('LocalStorageProvider', () => {
    it('should upload and download files correctly', async () => {
      ...
      // Upload file
      await storageManager.upload(testContent, { key, filename: 'hello.txt', contentType: 'text/plain' })
    ...
    it('should delete files correctly', async () => {
      ...
      // Upload file
      await storageManager.upload(testContent, { key, filename: 'delete-me.txt', contentType: 'text/plain' })
    ...
    it('should generate URLs correctly', async () => {
      ...
      // Upload file
      await storageManager.upload(testContent, { key, filename: 'url-test.txt', contentType: 'text/plain' })
  ...
  describe('Hybrid Storage Rules', () => {
    ...
    it('should route files based on path patterns', async () => {
      ...
      // Upload logo
      await storageManager.upload(logoContent, { key: logoKey, filename: 'company-logo.png', contentType: 'image/png' })
    ...
    it('should route files based on context', async () => {
      ...
      const provider = storageManager.determineStorageProvider('favicon.ico', 'image/x-icon', 'siteAssets')
      assert.strictEqual(provider, 'local')
      
      // Upload with context
      await storageManager.upload(assetContent, { key: assetKey, filename: 'favicon.ico', contentType: 'image/x-icon', context: 'siteAssets' })
    ...
    it('should fall back to available providers', async () => {
      ...
      const provider = storageManager.determineStorageProvider('file.pdf', 'application/pdf', 'ticketAttachments')
      ...
      // Upload attachment
      await storageManager.upload(attachmentContent, { key: attachmentKey, filename: 'file.pdf', contentType: 'application/pdf', context: 'ticketAttachments' })
  ...
  describe('Provider Management', () => {
    it('should handle missing providers gracefully', async () => {
      ...
      try {
        await storageManager.upload(testContent, { key, filename: 'test.txt', contentType: 'text/plain', storageProvider: 'nonexistent' })
