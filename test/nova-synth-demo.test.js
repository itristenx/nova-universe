...
describe('Nova Synth AI Escalation Demo', () => {
  test('should demonstrate AI escalation detection in action', async () => {
    ...
    const escalationScenarios = [
      ...
      },
      {
        ...
        ticket: {
          ...
          description:
            'Critical network outage affecting Buildings A, B, and C. All internet, WiFi, VoIP, and email services are completely down. Estimated 500+ users affected. Business operations halted.',
          category: 'network',
        },
      },
      {
        ...
        ticket: {
          ...
          description:
            'Multiple unauthorized access attempts detected on critical servers. Potential data breach in progress. Security team notified.',
          category: 'security',
        },
      },
    ];
    ...
    for (const scenario of escalationScenarios) {
      ...
      console.log(
        `   Confidence: ${Math.round((result.aiClassification?.confidence || 0) * 100)}%`,
      );
      console.log(`   Processing Time: ${endTime - startTime}ms`);

      if (result.duplicateAnalysis) {
        console.log(
          `   Duplicate Check: ${result.duplicateAnalysis.isDuplicate ? 'Duplicate detected' : 'Unique ticket'}`,
        );
      }
  ...
  test('should demonstrate duplicate detection for escalations', async () => {
    ...
    for (let i = 0; i < similarTickets.length; i++) {
      ...
      console.log(
        `   Duplicate Status: ${result.duplicateAnalysis?.isDuplicate ? 'DUPLICATE DETECTED' : 'Unique ticket'}`,
      );
  ...
  test('should demonstrate high-volume escalation processing', async () => {
    ...
    const escalationTypes = [
      ...
      },
      {
        ...
        desc: 'Production server down',
      },
      {
        category: 'software',
        priority: 'high',
        title: 'Application crash',
        desc: 'Critical application failure',
      },
    ];
    ...
    console.log(
      `   Successful AI Classifications: ${successfulClassifications}/${ticketCount} (${Math.round((successfulClassifications / ticketCount) * 100)}%)`,
    );
