// Basic Functionality Tests for Nova Universe
...
test('Basic System Tests', async (t) => {
  await t.test('Arithmetic operations', () => {
...
test('Business Logic Tests', async (t) => {
  await t.test('Priority calculation', () => {
    ...
    assert.strictEqual(calculatePriority(3, 3, 3), 'Medium'); // 3*10 + 3*5 + 3*3 = 30 + 15 + 9 = 54
    assert.strictEqual(calculatePriority(1, 1, 1), 'Low');

    // Test edge cases
    assert.strictEqual(calculatePriority(5, 5, 5), 'Critical'); // 5*10 + 5*5 + 5*3 = 50 + 25 + 15 = 90
    assert.strictEqual(calculatePriority(4, 4, 4), 'High'); // 4*10 + 4*5 + 4*3 = 40 + 20 + 12 = 72
    assert.strictEqual(calculatePriority(3, 3, 3), 'Medium'); // 3*10 + 3*5 + 3*3 = 30 + 15 + 9 = 54
  });
...
test('Utility Function Tests', async (t) => {
  ...
  await t.test('Date operations', () => {
    ...
    const createDateForDay = (dayOfWeek) => {
      const date = new Date();
    ...
    const monday = createDateForDay(1); // Monday = 1
    const saturday = createDateForDay(6); // Saturday = 6

    // Verify the dates are correct
    assert.strictEqual(monday.getDay(), 1); // Monday = 1
    assert.strictEqual(saturday.getDay(), 6); // Saturday = 6
